...
-- This SQL creates all tables needed for the School Admin application on Supabase
...
-- Create a policy allowing authenticated users to read audit logs (admins can see all)
-- Note: You'll need to set up proper RLS policies based on your authentication method
-- Example RLS policy (uncomment and adjust as needed):
/*
CREATE POLICY "audit_log_readable_by_admin" ON audit_log
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role = 'admin')
  );

CREATE POLICY "students_readable" ON students
  FOR SELECT USING (TRUE);

CREATE POLICY "staff_readable" ON staff
  FOR SELECT USING (TRUE);
*/
