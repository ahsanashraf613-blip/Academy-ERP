...
async function loadDB() {
    ...
    try {
        ...
        if (e8) throw e8;
        const { data: grades, error: e9 } = await supabaseClient.from('grades').select('*');
        if (e9) throw e9;
        ...
        document.getElementById('appContent').innerHTML = `
            <div class="panel" style="border-color: var(--danger);">
                ...
                <div class="panel-body">
                    ...
                    <p>Make sure you ran the latest SQL code to create the grades table!</p>
                </div>
            </div>
        `;
...
async function fetchAuditLog() {
    try {
        ...
        console.error("Fetch audit log error:", e.message);
    }
}
...
async function runPayroll(amount) {
    ...
    try {
        const { error } = await supabaseClient.from('ledger').insert([{ 
            ...
            desc: `Monthly Payroll - ${new Date().toLocaleDateString('en-PK', { month: 'long' })}`, type: "Expense", amount: amount 
        }]);
        if (error) throw error;
        await logAction(`Ran payroll for ${db.staff.length} staff. Total: ${fmt(amount)}`);
        await loadDB(); navigateTo('payroll');
        ...
        <div class="panel">
            ...
            <div class="panel-body">
                ...
                <div class="table-wrapper"><table id="stuTable"><thead><tr><th>ID</th><th>Name</th><th>Grade</th><th>Fee Status</th><th>Actions</th></tr></thead><tbody>
                    ${db.students.map(s => {
                        let bal = s.total - s.paid;
                        return `<tr><td>${s.id}</td><td style="font-weight:600;">${s.name}</td><td>${s.grade}</td><td style="color:${bal>0?'var(--danger)':'var(--success)'}; font-weight:600;">${fmt(bal)}</td>
                        <td>
                            <button class="btn btn-success btn-sm" onclick="openFeeModal('${s.id}')">Collect Fee</button>
                            <button class="btn btn-ghost btn-sm" onclick="openStudentEditModal('${s.id}')">Edit</button>
                            <button class="btn btn-danger btn-sm" onclick="deleteStudent('${s.id}')">Delete</button>
                        </td></tr>`;
                    }).join('') || '<tr><td colspan="5" style="text-align:center;">No students admitted yet.</td></tr>'}
                </tbody></table></div>
            </div>
        </div>
    `;
...
function renderStuTable() {
    ...
    document.querySelector('#stuTable tbody').innerHTML = filtered.map(s => {
        let bal = s.total - s.paid;
        return `<tr><td>${s.id}</td><td style="font-weight:600;">${s.name}</td><td>${s.grade}</td><td style="color:${bal>0?'var(--danger)':'var(--success)'}; font-weight:600;">${fmt(bal)}</td>
        <td>
            <button class="btn btn-success btn-sm" onclick="openFeeModal('${s.id}')">Collect Fee</button>
    ...
    }).join('') || '<tr><td colspan="5" style="text-align:center;">No students match your search.</td></tr>';
}
    ...
    return `
        <div class="panel">
            ...
            <div class="table-wrapper"><table><thead><tr><th>ID</th><th>Name</th><th>Role</th><th>Salary</th><th>Status</th><th>Actions</th></tr></thead><tbody>
                ${db.staff.map(s => `<tr><td>${s.id}</td><td style="font-weight:600;">${s.name}</td><td>${s.role}</td><td>${fmt(s.salary || 0)}</td><td><span class="badge badge-info">${s.status}</span></td>
                <td>
                    <button class="btn btn-ghost btn-sm" onclick="openStaffEditModal('${s.id}')">Edit</button>
                    <button class="btn btn-danger btn-sm" onclick="deleteStaff('${s.id}')">Delete</button>
                </td></tr>`).join('') || '<tr><td colspan="6" style="text-align:center;">No staff members found.</td></tr>'}
            </tbody></table></div>
        </div>
    `;
}
...
function getAttendanceHTML() {
    if (currentPortal === 'parent') {
        ...
        return `
            <div class="panel">
                <div class="panel-header"><h3>${child ? child.name : 'Child'}'s Attendance History</h3></div>
...
async function saveAttendance() {
    ...
    for (let p of list) {
        ...
        if (selectEl && !selectEl.disabled) {
            recordsToInsert.push({ id: Date.now() + Math.floor(Math.random() * 1000), date: todayISO, person_id: p.id, person_name: p.name, person_type: attendanceType, status: selectEl.value });
        }
    }
    ...
    try {
        ...
        await loadDB(); navigateTo('attendance');
...
function getAcademicsHTML() {
    ...
    return `
        <div class="panel">
            <div class="panel-header"><h3>Gradebook Management</h3>${(currentPortal === 'teacher' || currentPortal === 'admin') ? `<button class="btn btn-primary btn-sm" onclick="openGradeModal()">Add Grade</button>` : ''}</div>
        ...
        <div class="panel">
            ...
            <div class="panel-body">
                ${db.assignments.map(a => `<div style="padding:12px; border:1px solid var(--border); border-radius:8px; margin-bottom:10px;"><strong>${a.title}</strong> (Due: ${a.due})<br><span style="font-size:0.85rem; color:var(--text-muted);">${a.desc}</span></div>`).join('') || '<p>No assignments posted yet.</p>'}
            </div>
        </div>
    `;
}
...
function getTimetableHTML() {
    ...
    return `
        <div class="panel">
            <div class="panel-header"><h3>Dynamic Timetable</h3>${isAdmin ? `<button class="btn btn-primary btn-sm" onclick="openTimetableModal()">Add Slot</button>` : ''}</div>
            <div class="panel-body">
                <div class="search-bar"><label style="font-weight: 600; align-self: center;">Filter by Class:</label><select id="ttClassFilter" onchange="renderTimetableTable()"><option value="">All Classes</option>${uniqueClasses.map(c => `<option value="${c}">${c}</option>`).join('')}</select></div>
...
function getAnnouncementsHTML() {
    return `
        <div class="panel">
            ...
            <div class="panel-body">
                ${db.announcements.map(a => `<div style="padding:12px; border:1px solid var(--border); border-radius:8px; margin-bottom:10px;"><strong>${a.title}</strong> <span style="font-size:0.8rem; color:var(--text-muted);">(${a.date})</span><br>${a.desc}</div>`).join('') || '<p>No announcements posted yet.</p>'}
            </div>
        </div>
    `;
}
...
async function saveTransaction(type) {
    let desc = document.getElementById('transDesc').value; let amt = parseFloat(document.getElementById('transAmt').value);
    ...
    try {
        ...
        if (error) throw error;
        await logAction(`Logged ${type}: ${desc} (${fmt(amt)})`); closeModal(); await loadDB(); showToast(`${type} logged successfully!`);
    } catch (err) { showToast("Error: " + err.message, true); }
}
...
async function saveInventory() {
    let name = document.getElementById('invName').value; let stock = parseInt(document.getElementById('invStock').value);
    ...
    try {
        ...
        if (error) throw error; await logAction(`Added inventory item: ${name}`); closeModal(); await loadDB(); showToast("Item added successfully!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
function openInventoryEditModal(id) {
    let item = db.inventory.find(i => i.id === id); if(!item) return;
    openModal('Edit Inventory Item', `<div class="form-group"><label>Item Name</label><input class="form-control" id="editInvName" value="${item.item}"></div><div class="form-group"><label>Stock Quantity</label><input type="number" class="form-control" id="editInvStock" value="${item.stock}"></div><button class="btn btn-primary" style="width:100%" onclick="saveInventoryEdit('${id}')">Update Item</button>`);
}
async function saveInventoryEdit(id) {
    let name = document.getElementById('editInvName').value; let stock = parseInt(document.getElementById('editInvStock').value);
    if(!name || isNaN(stock) || stock < 0) return showToast("Invalid input", true);
    try {
        const { error } = await supabaseClient.from('inventory').update({ item: name, stock: stock, status: stock < 10 ? "Low Stock" : "In Stock" }).eq('id', id);
        if (error) throw error; await logAction(`Updated inventory item: ${name} (New Stock: ${stock})`); closeModal(); await loadDB(); showToast("Item updated successfully!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
async function deleteItem(id) { 
    if(!confirm("Are you sure you want to delete this item?")) return;
    try { const { error } = await supabaseClient.from('inventory').delete().eq('id', id); if (error) throw error; await logAction(`Deleted inventory item ID: ${id}`); await loadDB(); showToast("Item deleted."); } 
    catch (err) { showToast("Error: " + err.message, true); }
}

// --- STUDENT ACTIONS ---
function openStudentModal() {
    openModal('Admit New Student', `<div class="form-group"><label>Student Name</label><input class="form-control" id="stuName" placeholder="Full Name"></div><div class="form-group"><label>Grade / Class</label><input class="form-control" id="stuGrade" list="gradeOptions" placeholder="Type or select a grade"><datalist id="gradeOptions"><option value="10-A"><option value="9-B"><option value="12-A"><option value="11-C"><option value="8-A"></datalist></div><button class="btn btn-primary" style="width:100%" onclick="saveStudent()">Admit</button>`);
}
async function saveStudent() {
    let name = document.getElementById('stuName').value; let grade = document.getElementById('stuGrade').value;
    ...
    try {
        ...
        if (error) throw error; await logAction(`Admitted student: ${name} to ${grade}`); closeModal(); await loadDB(); showToast("Student admitted successfully!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
function openStudentEditModal(id) {
    let s = db.students.find(x => x.id === id); if(!s) return;
    openModal('Edit Student', `<div class="form-group"><label>Student Name</label><input class="form-control" id="editStuName" value="${s.name}"></div><div class="form-group"><label>Grade / Class</label><input class="form-control" id="editStuGrade" value="${s.grade}"></div><button class="btn btn-primary" style="width:100%" onclick="saveStudentEdit('${id}')">Update Student</button>`);
}
async function saveStudentEdit(id) {
    let name = document.getElementById('editStuName').value; let grade = document.getElementById('editStuGrade').value;
    if(!name || !grade) return showToast("Name and Grade are required", true);
    try {
        const { error } = await supabaseClient.from('students').update({ name, grade }).eq('id', id);
        if (error) throw error; await logAction(`Updated student ID: ${id}`); closeModal(); await loadDB(); showToast("Student updated successfully!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
async function deleteStudent(id) { 
    if(!confirm("Are you sure you want to delete this student?")) return;
    try { const { error } = await supabaseClient.from('students').delete().eq('id', id); if (error) throw error; await logAction(`Deleted student ID: ${id}`); await loadDB(); showToast("Student deleted."); } 
    catch (err) { showToast("Error: " + err.message, true); }
}
...
function openFeeModal(id) {
    let s = db.students.find(x => x.id === id); if(!s) return;
    let balance = s.total - s.paid;
    openModal('Collect Fee', `<p><strong>Student:</strong> ${s.name}<br><strong>Total Balance:</strong> ${fmt(balance)}</p><div class="form-group"><label>Amount to Collect (Rs.)</label><input type="number" class="form-control" id="feeAmt" value="${balance}" max="${balance}"></div><button class="btn btn-success" style="width:100%" onclick="saveFee('${id}')">Collect & Generate Receipt</button>`);
}
async function saveFee(id) {
    let s = db.students.find(x => x.id === id); if(!s) return;
    let amt = parseFloat(document.getElementById('feeAmt').value);
    if(!amt || amt <= 0 || amt > (s.total - s.paid)) return showToast("Invalid amount entered", true);

    try {
        // 1. Update Student Paid Amount
        const { error: e1 } = await supabaseClient.from('students').update({ paid: s.paid + amt }).eq('id', id);
        if (e1) throw e1;

        // 2. Log Income in Ledger
        const { error: e2 } = await supabaseClient.from('ledger').insert([{ id: Date.now(), date: new Date().toLocaleDateString(), desc: `Fee Collection - ${s.name} (${s.grade})`, type: "Income", amount: amt }]);
        if (e2) throw e2;

        await logAction(`Collected fee of ${fmt(amt)} from ${s.name}`);
        closeModal(); await loadDB(); showToast("Fee collected successfully!");
    } catch (err) { showToast("Error: " + err.message, true); }
}

// --- STAFF ACTIONS ---
function openStaffModal() {
    openModal('Staff Onboarding', `<div class="form-group"><label>Staff Name</label><input class="form-control" id="stfName"></div><div class="form-group"><label>Role</label><select class="form-control" id="stfRole"><option>Teacher</option><option>Admin</option><option>Accountant</option><option>Janitor</option><option>Security</option></select></div><div class="form-group"><label>Base Salary (Rs.)</label><input type="number" class="form-control" id="stfSalary" placeholder="e.g., 45000"></div><button class="btn btn-primary" style="width:100%" onclick="saveStaff()">Onboard</button>`);
}
async function saveStaff() {
    let name = document.getElementById('stfName').value; let role = document.getElementById('stfRole').value; let salary = parseInt(document.getElementById('stfSalary').value) || 0;
    ...
    try {
        ...
        if (error) throw error; await logAction(`Onboarded staff: ${name} as ${role}`); closeModal(); await loadDB(); showToast("Staff onboarded successfully!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
function openStaffEditModal(id) {
    let s = db.staff.find(x => x.id === id); if(!s) return;
    openModal('Edit Staff', `<div class="form-group"><label>Staff Name</label><input class="form-control" id="editStfName" value="${s.name}"></div><div class="form-group"><label>Role</label><select class="form-control" id="editStfRole"><option ${s.role==='Teacher'?'selected':''}>Teacher</option><option ${s.role==='Admin'?'selected':''}>Admin</option><option ${s.role==='Accountant'?'selected':''}>Accountant</option><option ${s.role==='Janitor'?'selected':''}>Janitor</option><option ${s.role==='Security'?'selected':''}>Security</option></select></div><div class="form-group"><label>Base Salary (Rs.)</label><input type="number" class="form-control" id="editStfSalary" value="${s.salary || 0}"></div><button class="btn btn-primary" style="width:100%" onclick="saveStaffEdit('${id}')">Update Staff</button>`);
}
async function saveStaffEdit(id) {
    let name = document.getElementById('editStfName').value; let role = document.getElementById('editStfRole').value; let salary = parseInt(document.getElementById('editStfSalary').value) || 0;
    if(!name) return showToast("Name required", true);
    try {
        const { error } = await supabaseClient.from('staff').update({ name, role, salary }).eq('id', id);
        if (error) throw error; await logAction(`Updated staff ID: ${id}`); closeModal(); await loadDB(); showToast("Staff updated successfully!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
async function deleteStaff(id) { 
    if(!confirm("Are you sure you want to delete this staff member?")) return;
    try { const { error } = await supabaseClient.from('staff').delete().eq('id', id); if (error) throw error; await logAction(`Deleted staff ID: ${id}`); await loadDB(); showToast("Staff member deleted."); } 
    catch (err) { showToast("Error: " + err.message, true); }
}

// --- ACADEMICS & GRADEBOOK ACTIONS ---
function openGradeModal() {
    let studentOptions = db.students.map(s => `<option value="${s.id}|${s.name}">${s.name} (${s.grade})</option>`).join('');
    openModal('Add Grade', `<div class="form-group"><label>Student</label><select class="form-control" id="gradeStudent">${studentOptions}</select></div><div class="form-group"><label>Subject</label><input class="form-control" id="gradeSubject" placeholder="e.g., Mathematics"></div><div class="form-group"><label>Marks (out of 100)</label><input type="number" class="form-control" id="gradeMarks" min="0" max="100"></div><button class="btn btn-primary" style="width:100%" onclick="saveGrade()">Save Grade</button>`);
}
async function saveGrade() {
    let studentVal = document.getElementById('gradeStudent').value.split('|');
    let subject = document.getElementById('gradeSubject').value; let marks = parseInt(document.getElementById('gradeMarks').value);
    if(!subject || isNaN(marks) || marks < 0 || marks > 100) return showToast("Invalid input", true);
    try {
        const { error } = await supabaseClient.from('grades').insert([{ id: Date.now(), student_id: studentVal[0], student_name: studentVal[1], subject, marks }]);
        if (error) throw error; await logAction(`Added grade for ${studentVal[1]} in ${subject}`); closeModal(); await loadDB(); showToast("Grade added successfully!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
async function deleteGrade(id) {
    if(!confirm("Are you sure you want to delete this grade?")) return;
    try { const { error } = await supabaseClient.from('grades').delete().eq('id', id); if (error) throw error; await logAction(`Deleted grade ID: ${id}`); await loadDB(); showToast("Grade deleted."); } 
    catch (err) { showToast("Error: " + err.message, true); }
}

function openAssignmentModal() {
    openModal('Post Assignment', `<div class="form-group"><label>Title</label><input class="form-control" id="asgTitle"></div><div class="form-group"><label>Description</label><textarea class="form-control" id="asgDesc"></textarea></div><div class="form-group"><label>Due Date</label><input type="date" class="form-control" id="asgDue"></div><button class="btn btn-primary" style="width:100%" onclick="saveAssignment()">Post</button>`);
}
async function saveAssignment() {
    let title = document.getElementById('asgTitle').value; if(!title) return showToast("Title required", true);
    try {
        ...
        if (error) throw error; await logAction(`Posted assignment: ${title}`); closeModal(); await loadDB(); showToast("Assignment posted successfully!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
...
async function saveTimetable() {
    try {
        const { error } = await supabaseClient.from('timetable').insert([{ id: Date.now(), day: document.getElementById('ttDay').value, time: document.getElementById('ttTime').value, class: document.getElementById('ttClass').value, subject: document.getElementById('ttSubject').value, teacher: document.getElementById('ttTeacher').value }]);
        if (error) throw error; await logAction(`Added timetable slot`); closeModal(); await loadDB(); showToast("Timetable slot added!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
function openTimetableEditModal(id) {
    let slot = db.timetable.find(t => t.id == id); if(!slot) return;
    let days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]; let dayOptions = days.map(d => `<option value="${d}" ${slot.day === d ? 'selected' : ''}>${d}</option>`).join('');
    openModal('Edit Timetable Slot', `<div class="form-group"><label>Day</label><select class="form-control" id="editTtDay">${dayOptions}</select></div><div class="form-group"><label>Time</label><input type="time" class="form-control" id="editTtTime" value="${slot.time}"></div><div class="form-group"><label>Class</label><input class="form-control" id="editTtClass" value="${slot.class}"></div><div class="form-group"><label>Subject</label><input class="form-control" id="editTtSubject" value="${slot.subject}"></div><div class="form-group"><label>Teacher</label><input class="form-control" id="editTtTeacher" value="${slot.teacher}"></div><button class="btn btn-primary" style="width:100%" onclick="saveTimetableEdit('${id}')">Update Slot</button>`);
}
async function saveTimetableEdit(id) {
    let day = document.getElementById('editTtDay').value; let time = document.getElementById('editTtTime').value; let className = document.getElementById('editTtClass').value; let subject = document.getElementById('editTtSubject').value; let teacher = document.getElementById('editTtTeacher').value;
    if(!className || !subject || !teacher) return showToast("All fields are required", true);
    try {
        const { error } = await supabaseClient.from('timetable').update({ day, time, class: className, subject, teacher }).eq('id', id);
        if (error) throw error; await logAction(`Updated timetable slot ID: ${id}`); closeModal(); await loadDB(); showToast("Timetable slot updated!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
async function deleteSlot(id) { 
    if(!confirm("Are you sure you want to delete this slot?")) return;
    try { const { error } = await supabaseClient.from('timetable').delete().eq('id', id); if (error) throw error; await logAction(`Deleted timetable slot`); await loadDB(); showToast("Slot deleted."); } 
    catch (err) { showToast("Error: " + err.message, true); }
}

// --- ANNOUNCEMENT ACTIONS ---
function openAnnouncementModal() {
    openModal('Create Announcement', `<div class="form-group"><label>Title</label><input class="form-control" id="annTitle"></div><div class="form-group"><label>Details</label><textarea class="form-control" id="annDesc"></textarea></div><button class="btn btn-primary" style="width:100%" onclick="saveAnnouncement()">Publish</button>`);
}
async function saveAnnouncement() {
    let title = document.getElementById('annTitle').value; if(!title) return showToast("Title required", true);
    try {
        ...
        if (error) throw error; await logAction(`Published announcement: ${title}`); closeModal(); await loadDB(); showToast("Announcement published!");
    } catch (err) { showToast("Error: " + err.message, true); }
}
